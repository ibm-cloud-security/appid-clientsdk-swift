...
//        authManager.registrationManager.preferenceManager.getJSONPreference(name: AppIDConstants.registrationDataPref).set([AppIDConstants.client_id_String : "someclient", AppIDConstants.JSON_REDIRECT_URIS_KEY : []] as [String:Any])
    ...
    func testLaunchChangePasswordAuthorizationUI_NO_IDToken() {
        let authManager = BluemixAppID.AuthorizationManager(oAuthManager: OAuthManager(appId: AppID.sharedInstance))
        ...
        authManager.launchChangePasswordUI(authorizationDelegate:delegate(res: "failure", expectedErr: "No identity token found."))
    
    }
    
    func testLaunchChangePassword_ID_Token_Not_Of_CD() {
        ...
        authManager.launchChangePasswordUI(authorizationDelegate:delegate(res: "failure", expectedErr: "The identity token was not retrieved using cloud directory idp."))
