...
# This script is used by Travis-CI to automatically release new versions of App ID.
...
set -ev
cd ~/Documents
# GITHUB_TOKEN required for Travis to have permissions to push to the App ID repository
...
version=$(grep -o 'version.*=.*[0-9]' IBMCloudAppID.podspec | cut -f 2 -d "'")
git fetch --tags
